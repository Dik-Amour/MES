...
// 获取行样式
const getRowClassName = (record: MoldingScheduleData) => {
...
const generateMockData = () => {
  ...
  if (viewType.value === 'daily') {
    // 日明细数据
    for (let i = 1; i <= 20; i++) {
      const planQuantity = Math.floor(Math.random() * 50) + 20
    ...
    // 周统计数据
    for (let i = 1; i <= 15; i++) {
      const planQuantity = Math.floor(Math.random() * 300) + 100
      const actualQuantity = Math.floor(Math.random() * planQuantity)
